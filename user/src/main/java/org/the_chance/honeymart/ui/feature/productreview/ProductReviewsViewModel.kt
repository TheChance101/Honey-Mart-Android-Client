...
class ProductReviewsViewModel @Inject constructor(
    ...
    private fun getProductReviews(productId: Long) {
        ...
        tryToExecutePaging(
            { productReviewsPagingUseCase(productId) },
    ...
    private fun onGetProductReviewsSuccess(productId: PagingData<Review>) {
        val mappedProductReviews = productId.map { it.toProductReviewsUiState() }
        _state.update {
            it.copy(
                isLoading = false,
